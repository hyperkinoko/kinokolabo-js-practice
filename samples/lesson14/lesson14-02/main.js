...
window.onload = () => {
  ...
  document.addEventListener('keydown', defaultKeyEvent);
...
function canMoveTo(col, row, direction) {
  for(const position of blockType.shape[direction]) {
    ...
    if(row + position[1] > -1) {
      if (fixedBlocks[row + position[1]][col + position[0]] != null) {
        return false;
      }
    }
  }
...
function gameOver() {
  ...
  // ボタンを押せなくする
  // keyEventのやり直し
  document.removeEventListener('keydown', defaultKeyEvent);
  context.clearRect(0, 0, (GAME_AREA_COL_MAX + 1) * CELL_SIZE, (GAME_AREA_ROW_MAX + 1) * CELL_SIZE);
  contextBg.clearRect(0, 0, (GAME_AREA_COL_MAX + 1) * CELL_SIZE, (GAME_AREA_ROW_MAX + 1) * CELL_SIZE);
  fixedBlocks = GAME_OVER;
  drawFixedBlock();
}
